...
class EventAction : public G4UserEventAction
{
  ...
  public:
    ...
    void AddDirePlusx(G4double ed)            		{fePlusx  += ed;} // If angle < 30 mrad
    void AddDirePlusy(G4double ed)            		{fePlusy  += ed;} // If angle < 30 mrad
    void AddDirePlusz(G4double ed)            		{fePlusz  += ed;} // If angle < 30 mrad
    void AddEnergyExitTargetSecondaryElectron(G4double edep)  {fEnergyExitTargetSecondaryElectron  += edep;}; // If angle < 30 mrad
    void AddDireMinusx(G4double ed)            		{feMinusx  += ed;} // If angle < 30 mrad
    void AddDireMinusy(G4double ed)            		{feMinusy  += ed;} // If angle < 30 mrad
    void AddDireMinusz(G4double ed)            		{feMinusz  += ed;} // If angle < 30 mrad
    ...
    void AddEnergyExitTargetBrems(G4double edep)   {fEnergyExitTargetBrems += edep;};
    void AddEnergyExitTargetNucl(G4double edep)   {fEnergyExitTargetNucl += edep;};
    ...
    void AddAngleBrems(G4double theta)             {if (fBoolBrems == 0)   { fAngleBrems  += theta; fBoolBrems = 1; } };
    void AddAngleNucl(G4double theta)             {if (fBoolNucl == 0)   { fAngleNucl  += theta; fBoolNucl = 1; } };
  ...
  private:
    ...
    G4double      fEnergyExitTargetBrems;
    G4double      fEnergyExitTargetNucl;
    ...
    G4double      fAngleBrems;
    G4double      fAngleNucl;
    
    G4double      fAnglee;
    
    G4double      fePlusx, fePlusy, fePlusz, feMinusx, feMinusy, feMinusz;

    G4int         fBoolPrim, fBoolSecPositron, fBoolSecElectron, fBoolSecMuonPlus, fBoolSecMuonMinus, fBoolCharged, fBoolChargedCut05GeV, fBoolChargedCut1GeV, fBooleIoni, fBoolePP1, fBoolePP2, fBooleIoniCut05GeV, fBoolePP1Cut05GeV, fBoolePP2Cut05GeV, fBooleIoniCut1GeV, fBoolePP1Cut1GeV, fBoolePP2Cut1GeV, fBoolBrems, fBoolNucl; //a boolean which becomes 1 when a primary or secondary, respectively, is found with angle < 30 mrad
