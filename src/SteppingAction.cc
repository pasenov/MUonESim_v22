  ...
  if (track->GetParentID() == 1)  {
	...
	if (post->GetStepStatus() == fGeomBoundary)   {
	   ...
	   if (track->GetLogicalVolumeAtVertex() == fDetectorconstruction->GetLTarget())  {
	      ...
	      if (processName == "muBrems")  {
	         fEventaction->AddAngleBrems(angleSec);
	      	 fEventaction->AddEnergyExitTargetBrems(ekinPost);
	      }
	   
	      if (processName == "muonNuclear")  {
	      	 fEventaction->AddAngleNucl(angleSec);
	      	 fEventaction->AddEnergyExitTargetNucl(ekinPost);
	      }
	      
	      if (particleDefinition == G4Positron::Definition())   {
                 
	         if (processName == "muIoni") processIDnoCuts = 1; //ionization
	         if (processName == "muPairProd") processIDnoCuts = 2; //pair production
	         if (processName == "muBrems") processIDnoCuts = 3; //Bremsstrahlung
	         if (processName == "muonNuclear") processIDnoCuts = 4; //nuclear interaction
	         analysisManager->FillH1(38, processIDnoCuts);
	      ...
	      if(particleDefinition == G4Positron::Definition())   {
	         ...
	         if ((angleSec < 30*mrad) && (ekinPost > 1.0*GeV) ) {
	            ...
	            if (processName == "muPairProd") {processID = 2; fEventaction->AddDirePlusx(momDirSx); fEventaction->AddDirePlusy(momDirSy); fEventaction->AddDirePlusz(momDirSz);} //pair production
	            if (processName == "muBrems") processID = 3; //Bremsstrahlung
	            if (processName == "muonNuclear") processID = 4; //nuclear interaction
	      ...
	      if(particleDefinition == G4Electron::Definition())   {
		    ...
		    << G4BestUnit(ekinPost, "Energy")
	         ...
	         if ((angleSec < 30*mrad) && (ekinPost > 1.0*GeV) ) {
	            ...
	            if (processName == "muIoni") processID = 1; //ionization
	            if (processName == "muPairProd") {processID = 2; fEventaction->AddDireMinusx(momDirSx); fEventaction->AddDireMinusy(momDirSy); fEventaction->AddDireMinusz(momDirSz);} //pair production
	            if (processName == "muBrems") processID = 3; //Bremsstrahlung
	            if (processName == "muonNuclear") processID = 4; //nuclear interaction
	            analysisManager->FillH1(11, processID);
